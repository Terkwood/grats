...
impl EntryButtonsView {
    ...
    fn view_current_entry_button(&self, emoji: &Emoji) -> Html {
        let default_all: Vec<Emoji> = DefaultEmoji::all()
            .iter()
        ...
        html! {
            ...
            { emoji.0.clone() }
            { if can_delete {
                html! { <button class="delete_entry_button" onclick={self.link.callback(move |_| Msg::DelButton(itc.clone()))}> { "DELETE 🗑" } </button>}
            } else {
                html! { <></> }
            }}
            </li>
